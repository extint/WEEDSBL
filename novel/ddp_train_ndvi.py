...
"""
Training script for NDVI-based crop-weed segmentation using U-Net with CRF
Dataset: WeedsGalore (3 classes: Background, Crop, Weed)
Input: NDVI + Filtered-NIR + Green (3 channels)
Features:
...
- Optional CRF post-processing during inference
"""
...
from ndvi_dataset_loader import create_ndvi_dataloaders
from unet_crf_model import create_unet_with_crf, get_model_info
...
def main():
    parser = argparse.ArgumentParser(
        description="Train NDVI-based Crop-Weed Segmentation (U-Net + CRF)"
    )
    ...
    parser.add_argument("--use_crf", action="store_true",
                        help="Enable CRF post-processing (for inference only)")
    ...
    parser.add_argument("--sigma_d", type=float, default=5.0,
                        help="Bilateral filter spatial std")
    parser.add_argument("--sigma_r", type=float, default=0.1,
                        help="Bilateral filter range std")

    # Augmentation
    parser.add_argument("--augment", action="store_true", default=True,
                        help="Enable advanced agricultural augmentations")
    parser.add_argument("--hue_shift", type=int, default=15)
    parser.add_argument("--sat_shift", type=int, default=30)
    parser.add_argument("--val_shift", type=int, default=20)
    ...
    parser.add_argument("--hsv_prob", type=float, default=0.7)
    parser.add_argument("--brightness_prob", type=float, default=0.5)
    ...
    parser.add_argument("--output_dir", type=str, default="./experiments_ndvi",
    ...
    if args.exp_name is None:
        ...
        crf_str = "crf" if args.use_crf else "nocrf"
        args.exp_name = f"ndvi_unet_{aug_str}_{crf_str}_{timestamp}"
    ...
    config_dict = {
        **vars(args),
        ...
        "dataset": "weedsgalore_ndvi",
        "input_channels": "NDVI + Filtered-NIR + Green"
    }
    with open(config_path, "w") as f:
        json.dump(config_dict, f, indent=2)

    print(f"\n{'='*70}")
    print(f"[INFO] NDVI-based Crop-Weed Segmentation Training")
    print(f"[INFO] Input: NDVI + Filtered-NIR + Green (3 channels)")
    ...
    # Dataloaders
    print("[INFO] Creating NDVI-based dataloaders...")
    train_loader, val_loader = create_ndvi_dataloaders(
    ...
    print(f"[INFO] Total parameters: {model_info['total_parameters']:,} "
          f"({model_info['total_parameters_M']:.2f}M)")
