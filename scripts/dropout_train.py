...
- NIR Dropout for robustness to missing NIR data
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Weedy Rice Segmentation Model (U-Net, U-Net_SA, or LightMANet)"
    )
    ...
    # NIR Dropout (NEW)
    parser.add_argument("--nir_dropout_prob", type=float, default=0.2,
                        help="Probability of dropping NIR channel during training (only works with --use_rgbnir)")
    ...
    parser.add_argument("--output_dir", type=str, default="./dropout_experiments",
    ...
    if args.exp_name is None:
        ...
        nir_drop_str = f"_nirdrop{args.nir_dropout_prob}" if args.nir_dropout_prob > 0 else ""
        args.exp_name = f"{args.model}_{channels}{nir_drop_str}_{timestamp}"
    ...
    if args.nir_dropout_prob > 0:
        print(f"[INFO] NIR Dropout enabled: {args.nir_dropout_prob}")
    print(f"{'='*60}\n")
    ...
    # Dataloaders (WITH NIR DROPOUT SUPPORT)
    ...
    train_loader, val_loader, test_loader = create_weedy_rice_rgbnir_dataloaders(
        ...
        nir_drop_prob=args.nir_dropout_prob  # NIR Dropout parameter added here
    )
