...
class CropWeedDataset(Dataset):
    ...
    def _read_mask(self, path):
        """
        Converts 3-class mask to binary crop/weed:
            0 = crop
            1 = weed
            255 = ignore (background)
        """
        ...
        cw_mask[m == 1]   = 0    # crop
        cw_mask[m == 2]   = 1    # weed
        cw_mask[m == 0]   = 255  # ignore background
        return cw_mask
    ...
    def __getitem__(self, idx):
        ...
        img_t  = torch.cat([rgb_t, nir_t], dim=0)                        # (4, H, W)
...
def compute_metrics(pred, target):
    ...
    for cls, name in zip([0, 1], ["crop", "weed"]):
        ...
        iou   = (intersection + 1e-6) / (union + 1e-6) if union > 0 else torch.tensor(0.0)
        acc   = intersection / tgt_c.sum() if tgt_c.sum() > 0 else torch.tensor(0.0)
...
def main():
    ...
    model = UNet(
        ...
        base_ch=64,
        out_channels=2,   # crop vs weed
    ).to(DEVICE)

    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[INFO] UNet parameters: {total_params / 1e6:.2f}M")
    ...
    for epoch in range(1, 101):
        ...
        if val_iou > best_iou:
            best_iou = val_iou
            torch.save(model.state_dict(), "best_crop_weed_unet.pth")
            print("  [✓] Saved best model → best_crop_weed_unet.pth")
