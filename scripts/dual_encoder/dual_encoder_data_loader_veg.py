...
class DualEncoderWeedyRiceDataset(Dataset):
    ...
    # Raw mask pixel values — change here if your dataset differs
    BG_VAL   = 0
    CROP_VAL = 1
    WEED_VAL = 2
    ...
    def __init__(
        ...
        mask_mode:
            "vegetation"  →  binary: (crop OR weed)=1, background=0
                             This is Stage 1 — what the model should learn first.
            "weed_only"   →  binary: weed=1, (background OR crop)=0
                             This is Stage 2 — weed vs crop+bg.
                             Only makes sense to train on vegetation pixels,
                             but kept as a flat mask here for simplicity.
        """
        assert mask_mode in ("vegetation", "weed_only"), \
            f"mask_mode must be 'vegetation' or 'weed_only', got {mask_mode}"
    ...
    def _read_mask(self, path):
        ...
        if self.mask_mode == "vegetation":
            # Stage 1: everything that is NOT background → 1
            binary = (m > self.BG_VAL).astype(np.uint8)   # crop=1, weed=1, bg=0
        else:
            # Stage 2: only weed → 1
            binary = (m == self.WEED_VAL).astype(np.uint8)

        return binary   # values strictly in {0, 1}
...
def create_dual_encoder_dataloaders(
    ...
    num_workers: int = 4,
    target_size: Tuple[int, int] = (512, 512),
    mask_mode: str = "vegetation",   # pass "weed_only" for Stage 2
):
    ...
    # Quick sanity-print so you can see what the mask looks like at startup
    sample_mask = train_ds[0]["mask"].numpy()
    unique_vals  = np.unique(sample_mask)
    veg_pct      = sample_mask.mean() * 100
    print(f"[DataLoader] mask_mode='{mask_mode}'  "
          f"unique values in first sample: {unique_vals}  "
          f"foreground={veg_pct:.1f}%")
