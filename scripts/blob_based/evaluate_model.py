...
def evaluate_pixel_metrics(model, dataset, device, class_names=['crop', 'weed']):
    """
    Calculate pixel-wise IoU and accuracy 
    USES EXACT SAME PIPELINE AS TRAINING: prepare_blob_samples()
    """
    ...
    print("Using EXACT TRAINING PIPELINE: NDVI → blobs → masking...")
    
    iou_per_class = {0: [], 1: []}  # crop, weed
    ...
    for img_id in tqdm(list(image_groups.keys()), desc="Processing images"):
        ...
        # Step 1: Compute NDVI and get vegetation mask
        ...
        # Step 4: Process each blob (SAME AS prepare_blob_samples)
        for blob in blobs:
            ys, xs = np.where(blob)
            if len(ys) == 0:
                continue
            ...
            # Get patch and blob crop (SAME AS TRAINING)
            patch = rgb[y1:y2+1, x1:x2+1].copy()
            blob_crop = blob[y1:y2+1, x1:x2+1]
            
            # Mask out non-vegetation pixels (SAME AS TRAINING)
            patch[blob_crop == 0] = 0
            ...
            # Prepare 4-channel input (RGB + NIR)
            ...
            with torch.no_grad():
                ...
                pred_class = torch.argmax(output, dim=1).item()  # 0=crop, 1=weed
            
            # Map prediction to pixels (ONLY vegetation pixels from blob_crop)
            # This is the key: only label pixels where blob_crop > 0
            pred_mask[y1:y2+1, x1:x2+1][blob_crop > 0] = pred_class + 1  # 1=crop, 2=weed
        
        # Step 5: Calculate pixel-wise metrics
        for class_id in [1, 2]:  # crop=1, weed=2 in masks
...
def main():
    print("\n" + "="*70)
    print("BBOX CLASSIFICATION + PIXEL-WISE EVALUATION")
    print("="*70)
    ...
    # Evaluate pixel-wise metrics
    print("\n" + "="*70)
    print("STEP 2: PIXEL-WISE EVALUATION (NDVI-based reconstruction)")
    print("="*70)
    seg_metrics = evaluate_pixel_metrics(model, val_ds, DEVICE)
    ...
    visualize_predictions(val_ds, model, DEVICE, num_samples=50,
